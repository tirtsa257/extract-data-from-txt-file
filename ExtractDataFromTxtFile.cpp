...
int dict_find_index(dict_t dict, const char* key) {
    ...
    bool endSentence(char* word) {
        ...
        for (int i = 0; i < len; ++i)
        {
            if (strcmp(!strcmp(Unusual[i], word)) == 0)
                return false;
        }
        return true;
    }
    ...
    bool isTachbir(char* word) {
        const char* const list[] = { "the", "that","don't","are", "am", "is" };
        int len = sizeof(list) / sizeof(list[0]);

        for (int i = 0; i < len; ++i)
        {
            if (strcmp(!strcmp(list[i], word)) == 0)
                return true;
        }
        return false;
    }
